...
func _process(delta: float) -> void:
	...
	var x_off := clamp(sway_offset.x, -sway_max_offset, sway_max_offset)
	var y_off := clamp(sway_offset.y, -sway_max_offset, sway_max_offset)
	position = initial_position + Vector3(x_off, y_off, current_recoil)
	var max_rot := deg_to_rad(sway_max_rot_deg)
